...
export default function ProcessDetailPage() {
  ...
  const processKPIs = kpis.filter((k) => k.processKeys?.includes((process?.key || key.toUpperCase()) as any))
  const processPainPoints = painPoints.filter((p) => p.processKeys?.includes((process?.key || key.toUpperCase()) as any))
  const processCapabilities = capabilities.filter((c) => c.processKeys?.includes((process?.key || key.toUpperCase()) as any))

  // Get related references from gallery
  const relatedReferences = galleryRegistry.references.filter((r) =>
    r.processKeys?.includes((process?.key || key.toUpperCase()) as any),
  )
  const relatedPatterns = galleryRegistry.patterns.filter((p) =>
    p.processKeys?.includes((process?.key || key.toUpperCase()) as any),
  )
