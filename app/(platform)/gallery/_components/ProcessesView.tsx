...
export default function ProcessesView() {
  ...
  const selectedProcessFacets = selectedProcessKey ? processFacets[selectedProcessKey] : null

  const getProcessIndustries = (processKey: string) => {
    return industries.filter((industry) => industry.processKeys?.includes(processKey))
  }

  const getProcessProducts = (processKey: string) => {
    const facets = processFacets[processKey]
    const productsGroup = facets?.groups.find((g) => g.key === "products")
    return productsGroup?.options || []
  }

  const getProcessVendors = (processKey: string) => {
    const facets = processFacets[processKey]
    const vendorsGroup = facets?.groups.find((g) => g.key === "vendor")
          ...
          return (
            ...
            >
              ...
              <div className="flex-1">
                <h3 className="font-semibold text-lg mb-3 gradient-heading">{process.sapLabel}</h3>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <div className="text-xs" style={{ color: "var(--text-muted)" }}>
                              {industry.counts?.references || 0} references
                            </div>
                          </div>
                        </div>
                      </div>
