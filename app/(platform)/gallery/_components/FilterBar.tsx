...
export default function FilterBar() {
  const { query, setQuery, filters, setFilters, clearFilters, industries, processes, capabilities } = useGalleryStore()
  ...
  return (
    <div className="sticky top-0 z-40 border-b bg-white/80 backdrop-blur-sm" style={{ borderColor: "var(--border)" }}>
              ...
              <div className="space-y-2">
                {["Low", "Medium", "High"].map((complexity) => (
                  <label key={complexity} className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                      checked={filters.complexity?.includes(complexity as any) || false}
